...
with, four independent axes combinable in any filter (full table in `README.md`'s "Running
tests" section):
...
```bash
mvn test -Dtest=... -Dmasking.enabled=true
# or: MASKING_ENABLED=true mvn test -Dtest=...
```
...
detail: masked request/response, steps, screenshots, page source, etc.):

```bash
mvn test -Dtest=... -Dreport.types=allure         # Allure only
mvn test -Dtest=... -Dreport.types=extent,allure  # both
```
...
- `reports/extent/index.html` — self-contained HTML report (or `reports/extent/report-{timestamp}.html`
  per run if `report.overwrite=false`)
...
| `-Dparallel=methods` API run shows one `status="SKIP"`/`retried="true"` test in `target/surefire-reports/testng-results.xml` but the run still reports `BUILD SUCCESS` | Not a bug — `RetryAnalyzer` (`retry.max.count`) absorbed a transient failure (a network blip against the live API) and the retry passed; TestNG's own bookkeeping convention records the failed first attempt as `SKIP`. Only a real `BUILD FAILURE` needs investigating. |
